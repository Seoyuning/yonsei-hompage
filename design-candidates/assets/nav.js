...
(function () {
  ...
  var MENU = [
    ...
    { t: '소식', h: 'G-news.html', key: 'news', sub: [['공지사항', 'G-news.html#notice', ['notice']], ['뉴스 · 연구성과', 'G-news.html#hi', ['hi']], ['세미나 · 행사', 'G-news.html#sched', ['sched']], ['학사 일정', 'G-news.html#calendar', ['calendar']], ['학위논문심사', 'G-news.html#thesis', ['thesis']], ['자료실', 'G-news.html#archive', ['archive']], ['취업 정보', 'G-news.html#jobs', ['jobs']]] },
  ...
  /* 같은 판을 다른 로딩에도 쓴다 — 관리자 열기(아래 런타임 블록)와
     로그인 뒤 도구 준비(studio/boot.js)가 문구만 바꿔 달아 띄운다.
     쪽 옮김용 8초 자동 숨김 대신 넉넉한 안전망만 두고, 끄는 건 부르는 쪽 몫. */
  try {
    window.YSME_LOADER = {
      show: function (text) {
        var el = loaderEl();
        el.querySelector('.yld-t').textContent = text || '로딩중';
        el.setAttribute('aria-label', text || '불러오는 중');
        requestAnimationFrame(function () { el.classList.add('on'); });
        clearTimeout(loaderTimer);
      ...
      hide: function () {
        hideLoader();
        if (yld) yld.querySelector('.yld-t').textContent = '로딩중';
      }
    };
  } catch (eL) {}
...
(function () {
  ...
  /* (3) 관리자 링크 가로채기 — 페이지를 다시 부르지 않고 그 자리에서 스튜디오를 깨운다.
     ...
     로딩 표시는 쪽 옮김과 같은 판(window.YSME_LOADER — 흰 원 + 마스코트 독수리
     날갯짓)을 문구만 바꿔 쓰고, 게이트나 도구 막대(.ys-gate/.ys-root)가 올라오면 걷는다. */
  try {
    document.addEventListener('click', function (e) {
      if (e.defaultPrevented || e.button !== 0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;
      var a = e.target && e.target.closest ? e.target.closest('a[href*="studio=1"]') : null;
      if (!a) return;
      var Ys = window.YStudio;
      if (Ys && Ys.boot) {
        /* 이미 부팅됐다(도구가 떠 있으면 이동만 막는다). 종료 후 재진입 등
           화면에 아무것도 없는 경우는 원래 링크대로 이동시켜 처음부터 다시 연다. */
        if (document.querySelector('.ys-gate, .ys-root')) e.preventDefault();
        return;
      }
      e.preventDefault();
      var L = window.YSME_LOADER;
      if (L) L.show('관리자 도구를 여는 중');
      ...
      (function tick() {
        ...
        if (up || dead || Date.now() - t0 > 25000) { if (L) L.hide(); return; }
        setTimeout(tick, 120);
      })();
    });
  } catch (e4) {}
