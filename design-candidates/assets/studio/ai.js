...
(function () {
  ...
  function jump(ch, jumpBtn) {
    if (isHere(ch)) {
      ...
      /* **위치만** 보여 준다 — 변경안 목록(승인 버튼)을 그대로 두고,
         그 자리를 클릭했을 때 편집 패널이 열린다. 찾아 바꾸기 쪽과 같은 규칙이다. */
      if (Y.hud && Y.hud.revealIdx) {
        ...
        Y.toast('표시한 자리를 클릭하면 편집 칸이 열립니다.');
      }
      return;
    }
    var page = pageOf(ch);
  ...
  function goToHit(ch, path, hit) {
    ...
    /* **위치만** 보여 준다. 목록은 그대로 두고, 그 자리를 클릭했을 때 편집 패널이 열린다.
       (편집 모드는 켜 둔다 — 그래야 클릭이 편집으로 이어진다) */
    if (Y.hud && Y.hud.revealIdx) {
      Y.hud.revealIdx(idx, {
        arm: true,
