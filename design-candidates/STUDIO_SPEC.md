...
쪽 옮김 로딩 판(.yld — 흰 원 + 마스코트 독수리 부품 그림 날갯짓)을 문구만 바꿔 띄운 뒤
boot.js 를 붙이고, 게이트/도구(.ys-gate/.ys-root)가 화면에 오르면 걷는다. 이 판은
`window.YSME_LOADER = { show(text), hide() }` 로 공개되어 boot.js 가 로그인 뒤 도구 준비
동안에도 쓰고, hud.js 진행 알약(.ys-spin)은 같은 독수리의 축소판을 쓴다 — 사이트의
로딩 표시는 이 마스코트 독수리 하나로 통일. 링크를 누르기 전에는 여전히 아무것도 로드하지 않는다.
...
| `모바일` | 모바일 렌더 모드 토글 |
| `한/영` | 편집 대상 언어 전환 |
| `깃헙` | 깃헙 관리 패널 — 연결 카드·연결 다시 확인(기준점 재동기)·최근 커밋(게시 취소/재게시, 취소 기록·취소된 게시 표식)·**연결 변경**(열면 「직접 추가하기 | 링크 붙여넣기」 두 방법 버튼만. 직접 = 현재 값이 채워진 칸 전부, 링크 = 주소 칸 하나 → https/SSH 주소나 `owner/repo` 를 읽어 내면 그때 칸들이 채워진 채 등장. `ghadd use:true` 재사용 — 검증 후 즉시 전환)·기본 복귀(`ghreset`). ghlist/ghuse/ghdel 은 서버에 남아 있으나 화면은 변경·복귀만 쓴다 |
