...
기본 동작은 「사람마다 자기 API 키를 브라우저에 등록」이다. 시연 때는 그 단계가 거슬리므로,
**서버에 키를 심어 두면 공용 암호만 입력해도 곧바로 AI 수정이 열린다.**

Vercel 프로젝트 → Settings → Environment Variables 에 넣는다(저장소에는 절대 넣지 않는다):

| 변수 | 값 | 비고 |
|---|---|---|
| `GEMINI_API_KEY` | aistudio.google.com 발급 키 | 무료 등급이면 요금 사고가 없다 |
| `ANTHROPIC_API_KEY` | (Claude 도 쓸 거면) | 없어도 된다 |
| `AI_RATE_MAX` | 기본 `100` | IP 당 창(window) 안에서 허용할 호출 수 |
| `AI_RATE_WINDOW_MS` | 기본 `300000` | 창 길이(밀리초) = 5분 |

기본값은 **IP 당 5분에 100회**다. 심사위원들이 같은 와이파이(= 같은 IP)에서 동시에 눌러봐도
넉넉하고, 암호가 새어 나갔을 때의 폭주는 여전히 막힌다. 두 변수는 넣지 않아도 되며,
값을 바꿀 때는 **환경변수를 고친 뒤 재배포해야** 반영된다(코드와 같은 규칙).

넣고 **재배포해야** 반영된다. 그 뒤 동작은 이렇다:

- 로그인하면 스튜디오가 서버에 "키 있나?" 만 묻는다(`{probe:true}`). **키 값은 브라우저로 내려오지 않는다.**
- 키가 있으면 등록 화면을 건너뛰고 바로 요청 화면이 뜬다. AI 패널에 「서버 키」 표시가 붙는다.
- 개인 키를 등록해 둔 사람은 **개인 키가 우선**한다. 「키 변경 → 키 삭제」 하면 서버 키로 돌아온다.

> ⚠️ 이렇게 하면 **공용 암호가 API 요금을 지키는 유일한 방어선**이 된다
> (그 암호는 이미 `/api/publish` 로 저장소 커밋 권한도 쥐고 있다).
> 시연장에서 암호를 타이핑하는 화면이 보일 수 있으니 **시연이 끝나면 암호를 교체**한다.
> 호출 제한은 서버리스 인스턴스 메모리 기반이라 인스턴스가 여러 개면 그만큼 느슨하다 —
> 과금 사고를 막는 속도 방지턱이지 완전한 방어가 아니다.
