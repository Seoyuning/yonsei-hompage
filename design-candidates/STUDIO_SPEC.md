...
| `모바일` | 모바일 렌더 모드 토글 |
| `한/영` | 편집 대상 언어 전환 |
| `깃헙` | 깃헙 관리 패널 — 연결 카드·연결 다시 확인(기준점 재동기)·최근 커밋(게시 취소/재게시, 취소 기록·취소된 게시 표식)·**연결 변경**(현재 값이 채워진 폼 + 맨 위 「깃헙 주소로 채우기」 — https/SSH 주소나 `owner/repo` 를 붙여넣으면 소유자·저장소·`/tree/브랜치/폴더` 까지 자동 채움. `ghadd use:true` 재사용 — 검증 후 즉시 전환)·기본 복귀(`ghreset`). ghlist/ghuse/ghdel 은 서버에 남아 있으나 화면은 변경·복귀만 쓴다 |
