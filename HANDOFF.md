...
- **자료실(→소식) · 학위논문심사(→대학원) 신설** — 원본 me.yonsei 를 조사해 실제 절차·서식·
  카테고리까지 채운다(실데이터). data.js 에 관련 데이터가 없어 원본 조사가 선행돼야 한다.
- **취업정보를 입학 → 소식 으로 이동** — 실제 채용공고 30건(`jobs`). 현재 `G-admissions#jobs` 에 있다.
