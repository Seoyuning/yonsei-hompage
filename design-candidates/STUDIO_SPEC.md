...
원형 독수리 로더(흰 원 + 도는 링 + 날갯짓, `window.YSME_LOADER`)를 띄운 뒤 boot.js 를 붙이고,
게이트/도구(.ys-gate/.ys-root)가 화면에 오르면 걷는다. 같은 로더를 boot.js 가 로그인 뒤
도구 준비 동안, hud.js 진행 알약(.ys-spin)이 축소판으로 쓴다 — 로딩 표시는 이것 하나로 통일.
링크를 누르기 전에는 여전히 아무것도 로드하지 않는다.
